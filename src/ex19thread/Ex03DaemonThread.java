...
class NotThread
{
	...
	void notThreadMethod() {
		for(int i =1 ; i<=10 ; i++) {
			System.out.println(String.format("%s]i=%d", 
					title, i));
		}
	}
}
...
/*
 독립쓰레드 (Non DaemonThread)
 	: 메인쓰레드와 working쓰레드(개발자가 정의한 쓰레드)을 일컫는 것으로 메인쓰레드가 종료되어도 계속
 	실행되며, Dead상태가 되어야 실행이 종료되는 쓰레드를 말한다.
 	해당 쓰레드를 실행하게 되면 main메소드가 조욜된 이후에도 실행되는것을 볼 수 있다.
 */
...
/*
 종속쓰레드(DaemonThread) extends Thread
	:
	모든 독립쓰레드가 종료되면 자동으로 종료되는 쓰레드를 말한다. 주로 프로그램의 보조역할로 배경음악,
	로그자동저장 등의 업무를 처리한다.
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		yt1.setName("첫번쨰 쓰레드");//쓰레드의 이름지정 두번째 방법
		...
		System.out.println("현재활성화된 상태의 쓰레드 수"+ 
				Thread.activeCount());
