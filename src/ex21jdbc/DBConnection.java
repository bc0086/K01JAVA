	...
	public static void main(String[] args) 
		...
		/*
		 ...
		 1] 오라클용 JDBC드라이버를 메모리에 로드함
		 	: new를 사용하지 않고 클래스명으로 직접 찾아서 객체생성 후 메모리에 로드하는
		 	forName()메소드를 사용한다. 메모리에 로딩된 드라이버가 DriverManager라는
		 	클래스에 등록된다.
		 */
		...
		/*
		 2-1] DriverManager클래스의 getConnection()메소드를 호출하여 오라클에 
		 연결을 시도한다. 연결에 성공할 경우 연결된 주소를 반환한다.
		 */
		Connection con
		...
		if(con!=null) {
			...
			/*
			 3] 쿼리문 작성
			 	: 쿼리문을 작성할 때 주의할 점은 줄바꿈을 할 때 앞뒤로 스페이스를 삽입하는것이 좋다
			...
			/*
			 5]Statement객체를 사용하여 오라클로 작성된 커리문을 전송한다.
			...
			while(rs.next()	) {
				...
				/*
				 ...
				 getXXX()계열의 함수의 인자로는 select절의 순서대로 '인덱스'를 쓸수도 있고,
				 '컬럼명'을 쓸수도 있다.
				 */
