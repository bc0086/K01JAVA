...
class Account {
	/*
	 멤버변수가 상속과계에 있다 하더라도 private으로 선언되면 클래스 내부에서만 접근가능하다.
	 즉 상속받은 하위클래스에서 직접접근은 불가능하다.
	 */
	...
	protected void depositMoney(int _money) {
		if(_money < 0) {
			System.out.println("마이너스 금액은 밉금처리 불가합니다.");
			return;
		}
		money += _money;
	}
	//private멤버변수를 외부클래스로 반환할떄 사용
...
class SavingAccount extends Account {
	/*
	 인자생성자에서 부모의 생성자 메소드를 호출하기 위해 super()를 사용하고 있다. 이때 인자가 하나인 생성자를
	 호출하게 된다.
	 */
	...
	/*
	 ...
	 private멤버변수인 money는 직접 접근할 수 없으므로 접근가능한 메소드를 톨해 간접적으로 접근한다.
	 */
