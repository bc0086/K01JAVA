...
class Man {
	...
	private String name;
	/*
	 만약 자식클래스에서 super()를 통해 매개변수를 전달하지 않는다면 초기화가 되지 않아 null이 출력될 수 있다.
	 */
	public Man() {
...
class BusinessMan extends Man {
	...
	/*
	 상속받은 자식클래스에서는 부모클래스의 생성자를 호출하여 부모객체가 먼저 생성되도록 해야하는 책임을 가지고 있다.
	 super()는 부모클래스의 생성자를 호출하는 역할을 하게되면 만약 super()를 쓰지 않으면 디폴트생성자가 호출된다.
	 */
	...
	public void tellYourInfo() {
		...
		//부모클래스에 정의된 멤버메소드 호출, 상속받았으므로 호출가능
		tellYourName();
	}
}
