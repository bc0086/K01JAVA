...
/*
 ...
 오버로딩 : 함수명은 같으나 매개변수의 갯수, 타입 , 순서가 다른경우를 말한다. 즉, 서로 다른 함수이므로
 		상속받은 하위클래스에는 오버로딩된 모든 메소드가 포함된다.
 		
 오버라이딩 : 함수명, 매개변수, 반환형까지 모두 동일한 함수를 상속관계에 있는 하위클래스에서 재정의 한 것을
 		말한다. 이 경우 하위클래스에서 정의한 메소드가 상위클래스의 메소드를 가리게 되므로 항상
 		최하위 클래스의 메소드가 호출되게 된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		/*
		 참조변수에 상관없이 항상 최하위에 오버라이딩 된 메소드가 호출된다. 즉 아래 부분운 모두 c클래스에 
		 생성된 메소드가 호출된다.
		 */
		...
		A refId1 = new C();
		/*
		 refId1은 A타입의 참조변수이므로 B타입 혹은 C타입의 참조변수로 참조 할 수 없다.
		 자식 타입의 참조변수로 부모객체를 참조하는것은 불가능하다.
		 */
		//B refId2 = refId1;
		//C refId3 = refId1;
