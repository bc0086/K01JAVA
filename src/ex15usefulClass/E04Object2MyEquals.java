...
/*
 equals()메소드
 -자바에서 인스턴수의 내용비교를 위해 비교연산자(==)를 사용하는 것은 단순히 참조값에 대한 비교이므로
 실제 내용에 대한 비교는 이뤄지지 않는다.
 -인스턴스간의 내용비교를 위해서는 equals()메소드를 각 클래스에 맞게 적절히 오버라이딩하려 사용해야한다.
...
class IntNumber {
	 ...
	 */
	public boolean myEquals(IntNumber num0bj) {
		if(this.num==num0bj.num) {
