...
/*
 static블럭
 	: 동일 클래스 안의 main메소드보다 먼저 실행되는 블럭으로 main메소드의 실행코드가 없어도 먼저 실행된다
 	단, 다른 클래스에 main이 있는 경우에는 main이 순차적으로 실행되다가 static블럭이 있는 클래스가 
 	객체화될때 실행하게된다. 이때 생성자보다 static블럭이 먼저 실행된다.
 */
public class StaticBlock {
	...
	static {
		...
		/*
		 static 블럭안에선만 사용할 수 있는 지역변수 선언.
...
//		System.out.println("==메인메소드==");
		...
		//static블럭 내에서만 선언된 지역변수이므로 main에서 사용불가
