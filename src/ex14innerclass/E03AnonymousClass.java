...
/*
 ...
 형식]
 	...
 	※부모클래스를 상속받아 이름없는 자식 클래스를 생성하는 개념으로 이해하면 된다. 상속되기 때문에 오버라이딩이
 	가능하게 된다.
 */
class Person {
	String name;
...
public class E03AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 * Person클래스를 상속받은 익명클래스 정의 및 생성 : Person타닙의 인스턴스 변수에 Person을 상속받은 익명클래스를 생성해서 그
		 * 주소를 할당한다. 개념적으로 new Person(){}; ==> 익명 extends Person{}; 형태라 생각하면 된다. -이름이 없는
		 * 객체이므로 부모클래스의 이름을 빌려서 생성했다고 생각하자.
		 */
		...
		/*
		 * 다운캐스팅은 "(자식클래스명)부모타입의 참조변수" 형태로 형변환하여 하위클래스의 멤버에 접근 하기 위한 수단이다. 그러나 익명클래스는
		 * 클래스명이 없기 때문에 위와 같이 다운캐스팅이 불가능하다. 즉, 익명클래스에서는 새롭게 정의한 멤버변수를 접근 할 수 없다. 따라서
		 * 익명클래스는 오버라이딩의 목적으로만 사용해야한다.
		 */
		/*
		 * Person annoyTest = new Student("다운캐스팅", "안되요") { String jumin = "001024"; };
		 * ((Student)annoyTest).jumin = "181105";
		 */
